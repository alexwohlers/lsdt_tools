...
def main() -> None:
    ...
    if args.time_file and args.fft_file:
        ...
        if args.time_file:
            ...
            if os.path.isabs(base_name) or base_name.startswith("measurements"):
                ...
                time_file = os.path.join("..", "data", "measurements", f"{base_name}.csv")
        ...
        if args.fft_file:
            ...
            fft_file = os.path.join("..", "data", "fft", f"{file_stem}.csv")
    ...
    if args.save_path:
        ...
        if not os.path.dirname(args.save_path):
            save_path = os.path.join("..", "bilder", "plot_fft", args.save_path)
        else:
            save_path = args.save_path
    else:
        # Standard: plot_fft/<name>.png
        save_path = os.path.join("..", "bilder", "plot_fft", f"{args.name}.png")
